...
===Using Styles (colors, font-size, etc)
There are a few extra parameters to include in your options hash.
1. Specify your styles, see below for an example
2. Specify the locations of where to apply those styles. You can specify a specific row or column. Columns can be specified by either name (symbol) or integer.
Note that both :row and :column are hashes within :locations. Individual cell support is not available yet.
